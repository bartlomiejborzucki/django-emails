...
class Email(models.Model):
    ...
    user                = models.ForeignKey(User, related_name = "email_from_user", null = True)
    creation_date       = models.DateTimeField(auto_now_add=True, editable=False, db_column='this_creation_date') # data dodania
    send_at             = models.DateTimeField(blank=True, null=True)
    email               = models.EmailField(blank = True, null = True)
    is_send_by_S3       = models.BooleanField(default = False)
    is_send             = models.BooleanField(default = True)
    #kiedy jest nullem wysyłamy z domyślnej skrzynki
    ...
    def __unicode__(self):
        return unicode(u'Tytuł: ') + unicode(self.topic) + smart_unicode(' do: ') + unicode(self.user)
